...
type HandDebugLane = 'LEFT' | 'CENTER' | 'RIGHT' | 'NO HAND'
...
export class MediaPipeHandInput implements InputController {
  ...
  update(_dtSeconds: number): void {
    ...
    const lane = this.resolveLaneFromResult(result.landmarks[0])
    ...
    this.updateDebugLabel(lane.toUpperCase() as HandDebugLane)
  ...
  private resolveLaneFromResult(landmarks: { x: number }[] | undefined): HitAction | null {
  ...
  private updateDebugLabel(lane: HandDebugLane): void {
    if (!this.laneLabelEl) return
    this.laneLabelEl.textContent = `HAND: ${lane}`
  }
